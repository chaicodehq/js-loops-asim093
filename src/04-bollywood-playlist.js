...
export function buildPlaylist(songs, maxDuration) {
  if (!Array.isArray(songs) || typeof maxDuration !== "number" || !isFinite(maxDuration) || maxDuration <= 0) {
  ...
  while (i < songs.length) {
    ...
    if (typeof song !== 'number' || !isFinite(song) || song <= 0) {
      i++;
      continue;
    }
  ...
  return { count: count, totalDuration: total }
}
